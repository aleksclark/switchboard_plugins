...
pub extern "C" fn metadata() -> u64 {
    sdk::leaked_metadata(&sdk::PluginMetadata {
        ...
        version: "0.1.0".into(),
        abi_version: 1,
        description: "Readarr book collection manager — manage authors, books, search, queue, and library".into(),
        ...
        placeholders: HashMap::from([
            ("api_key".into(), "your-readarr-api-key".into()),
            ("base_url".into(), "http://localhost:8787".into()),
        ]),
    })
}
...
pub extern "C" fn configure(ptr_size: u64) -> u64 {
    ...
    if key.is_empty() {
        return sdk::leaked_string("readarr: api_key is required");
    }
    let bu = creds
        .get("base_url")
        ...
        .unwrap_or_default();
    if bu.is_empty() {
        return sdk::leaked_string("readarr: base_url is required");
    }
...
fn dispatch(tool_name: &str, args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let handler: Option<HandlerFn> = match tool_name {
        // Authors
        "readarr_list_authors" => Some(list_authors),
        "readarr_get_author" => Some(get_author),
        "readarr_add_author" => Some(add_author),
        "readarr_update_author" => Some(update_author),
        "readarr_delete_author" => Some(delete_author),
        "readarr_lookup_author" => Some(lookup_author),
        // Books
        "readarr_list_books" => Some(list_books),
        "readarr_get_book" => Some(get_book),
        "readarr_add_book" => Some(add_book),
        "readarr_update_book" => Some(update_book),
        "readarr_delete_book" => Some(delete_book),
        "readarr_lookup_book" => Some(lookup_book),
        // Calendar
        "readarr_get_calendar" => Some(get_calendar),
        // Queue
        "readarr_get_queue" => Some(get_queue),
        "readarr_delete_queue_item" => Some(delete_queue_item),
        // History
        "readarr_get_history" => Some(get_history),
        // Commands
        "readarr_list_commands" => Some(list_commands),
        "readarr_run_command" => Some(run_command),
        "readarr_get_command" => Some(get_command),
        // System
        "readarr_get_status" => Some(get_status),
        // Quality Profiles
        "readarr_list_quality_profiles" => Some(list_quality_profiles),
        // Metadata Profiles
        "readarr_list_metadata_profiles" => Some(list_metadata_profiles),
        // Root Folders
        "readarr_list_root_folders" => Some(list_root_folders),
        // Tags
        "readarr_list_tags" => Some(list_tags),
        "readarr_create_tag" => Some(create_tag),
        "readarr_delete_tag" => Some(delete_tag),
        // Blocklist
        "readarr_get_blocklist" => Some(get_blocklist),
        "readarr_delete_blocklist_item" => Some(delete_blocklist_item),
        // Wanted/Missing
        "readarr_get_wanted_missing" => Some(get_wanted_missing),
        _ => None,
    };
...
fn do_get(path: &str) -> Result<String, String> {
    ...
    if resp.status >= 400 {
        return Err(format!("readarr API error ({}): {}", resp.status, resp.body));
    }
    Ok(resp.body)
}
...
fn list_authors(_args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    match do_get("/api/v1/author") {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}

fn get_author(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let id = match require_arg(&args, "id") { Ok(v) => v, Err(r) => return r };
    match do_get(&format!("/api/v1/author/{}", encode_path(&id))) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}

fn add_author(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let body = match parse_json_arg(&args, "body") {
        Ok(Some(v)) => v,
        Ok(None) => return sdk::err_result("body is required"),
        Err(r) => return r,
    };
    match do_post("/api/v1/author", &body) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}

fn update_author(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let id = match require_arg(&args, "id") { Ok(v) => v, Err(r) => return r };
    let body = match parse_json_arg(&args, "body") {
        Ok(Some(v)) => v,
        Ok(None) => return sdk::err_result("body is required"),
    ...
    match do_put(&format!("/api/v1/author/{}", encode_path(&id)), &body) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}

fn delete_author(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let id = match require_arg(&args, "id") { Ok(v) => v, Err(r) => return r };
    ...
    match do_delete(&format!("/api/v1/author/{}{}", encode_path(&id), qs)) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}

fn lookup_author(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let term = match require_arg(&args, "term") { Ok(v) => v, Err(r) => return r };
    let qs = build_query(&[("term", term)]);
    match do_get(&format!("/api/v1/author/lookup{}", qs)) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}
...
fn list_books(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let author_id = sdk::arg_str(&args, "author_id");
    let qs = build_query(&[("authorId", author_id)]);
    match do_get(&format!("/api/v1/book{}", qs)) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}

fn get_book(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let id = match require_arg(&args, "id") { Ok(v) => v, Err(r) => return r };
    match do_get(&format!("/api/v1/book/{}", encode_path(&id))) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}

fn add_book(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let body = match parse_json_arg(&args, "body") {
        Ok(Some(v)) => v,
        Ok(None) => return sdk::err_result("body is required"),
        Err(r) => return r,
    };
    match do_post("/api/v1/book", &body) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}

fn update_book(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let id = match require_arg(&args, "id") { Ok(v) => v, Err(r) => return r };
    let body = match parse_json_arg(&args, "body") {
        Ok(Some(v)) => v,
        Ok(None) => return sdk::err_result("body is required"),
    ...
    match do_put(&format!("/api/v1/book/{}", encode_path(&id)), &body) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}

fn delete_book(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let id = match require_arg(&args, "id") { Ok(v) => v, Err(r) => return r };
    let mut query_params: Vec<(&str, String)> = Vec::new();
    if sdk::arg_bool(&args, "delete_files") == Some(true) {
        query_params.push(("deleteFiles", "true".into()));
    ...
    let qs = build_query(&query_params);
    match do_delete(&format!("/api/v1/book/{}{}", encode_path(&id), qs)) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}

fn lookup_book(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let term = match require_arg(&args, "term") { Ok(v) => v, Err(r) => return r };
    let qs = build_query(&[("term", term)]);
    match do_get(&format!("/api/v1/book/lookup{}", qs)) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}
