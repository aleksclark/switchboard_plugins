...
pub extern "C" fn metadata() -> u64 {
    sdk::leaked_metadata(&sdk::PluginMetadata {
        ...
        version: "0.1.0".into(),
        abi_version: 1,
        description: "Sonarr TV series management — search, add, update, and manage TV shows, episodes, quality profiles, downloads, and more".into(),
        ...
        placeholders: HashMap::from([
            ("api_key".into(), "your-sonarr-api-key".into()),
            ("base_url".into(), "http://localhost:8989".into()),
        ]),
    })
}
...
pub extern "C" fn configure(ptr_size: u64) -> u64 {
    ...
    if key.is_empty() {
        return sdk::leaked_string("sonarr: api_key is required");
    }
    let bu = creds
        .get("base_url")
        ...
        .unwrap_or_default();
    if bu.is_empty() {
        return sdk::leaked_string("sonarr: base_url is required");
    }
...
fn dispatch(tool_name: &str, args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let handler: Option<HandlerFn> = match tool_name {
        // Series
        "sonarr_list_series" => Some(list_series),
        "sonarr_get_series" => Some(get_series),
        "sonarr_add_series" => Some(add_series),
        "sonarr_update_series" => Some(update_series),
        "sonarr_delete_series" => Some(delete_series),
        "sonarr_lookup_series" => Some(lookup_series),
        // Episodes
        "sonarr_get_episodes" => Some(get_episodes),
        "sonarr_get_episode" => Some(get_episode),
        // Calendar
        "sonarr_get_calendar" => Some(get_calendar),
        // Queue
        "sonarr_get_queue" => Some(get_queue),
        "sonarr_delete_queue_item" => Some(delete_queue_item),
        // History
        "sonarr_get_history" => Some(get_history),
        // Commands
        "sonarr_list_commands" => Some(list_commands),
        "sonarr_run_command" => Some(run_command),
        "sonarr_get_command" => Some(get_command),
        // System
        "sonarr_get_status" => Some(get_status),
        // Quality Profiles
        "sonarr_list_quality_profiles" => Some(list_quality_profiles),
        // Root Folders
        "sonarr_list_root_folders" => Some(list_root_folders),
        // Tags
        "sonarr_list_tags" => Some(list_tags),
        "sonarr_create_tag" => Some(create_tag),
        "sonarr_delete_tag" => Some(delete_tag),
        // Blocklist
        "sonarr_get_blocklist" => Some(get_blocklist),
        "sonarr_delete_blocklist_item" => Some(delete_blocklist_item),
        // Wanted/Missing
        "sonarr_get_wanted_missing" => Some(get_wanted_missing),
        _ => None,
    };
...
fn do_get(path: &str) -> Result<String, String> {
    ...
    if resp.status >= 400 {
        return Err(format!("sonarr API error ({}): {}", resp.status, resp.body));
    }
    Ok(resp.body)
}
...
fn list_series(_args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    match do_get("/api/v3/series") {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}

fn get_series(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let id = match require_arg(&args, "id") { Ok(v) => v, Err(r) => return r };
    match do_get(&format!("/api/v3/series/{}", encode_path(&id))) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}

fn add_series(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let body = match parse_json_arg(&args, "body") {
        Ok(Some(v)) => v,
        Ok(None) => return sdk::err_result("body is required"),
        Err(r) => return r,
    };
    match do_post("/api/v3/series", &body) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}

fn update_series(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let id = match require_arg(&args, "id") { Ok(v) => v, Err(r) => return r };
    let body = match parse_json_arg(&args, "body") {
        Ok(Some(v)) => v,
        Ok(None) => return sdk::err_result("body is required"),
    ...
    match do_put(&format!("/api/v3/series/{}", encode_path(&id)), &body) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}

fn delete_series(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let id = match require_arg(&args, "id") { Ok(v) => v, Err(r) => return r };
    ...
    match do_delete(&format!("/api/v3/series/{}{}", encode_path(&id), qs)) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}

fn lookup_series(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let term = match require_arg(&args, "term") { Ok(v) => v, Err(r) => return r };
    let qs = build_query(&[("term", term)]);
    match do_get(&format!("/api/v3/series/lookup{}", qs)) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}
...
fn get_episodes(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let series_id = match require_arg(&args, "series_id") { Ok(v) => v, Err(r) => return r };
    let qs = build_query(&[("seriesId", series_id)]);
    match do_get(&format!("/api/v3/episode{}", qs)) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}

fn get_episode(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    let id = match require_arg(&args, "id") { Ok(v) => v, Err(r) => return r };
    match do_get(&format!("/api/v3/episode/{}", encode_path(&id))) {
        Ok(d) => sdk::raw_result(d),
        Err(e) => sdk::err_result(&e),
    }
}
...
fn get_calendar(args: HashMap<String, serde_json::Value>) -> sdk::ToolResult {
    ...
    if sdk::arg_bool(&args, "include_series") == Some(true) {
        params.push(("includeSeries", "true".into()));
    }
